...
async function studentRoom(code,name){
  ...
  const render=()=>{
    ...
    shell(`<div class="topbar">${brand()}<span class="status-pill open"><i class="dot"></i>ההצבעה פתוחה</span></div><section class="card student-card"><div class="muted small">שאלה ${room.questionIndex+1} מתוך ${Q.length}</div><h2 class="questionIndex+1} מתוך ${Q.length}</div><h2 class="question-title" style="font-size:27px">${esc(q.question)}</h2><div class="student-options">${q.options.map((o,i)=>`<button class="student-option ${room.myVote===i?'selected':''}" data-answer="${i}"><span class="option-letter">${letters[i]}</span><span>${esc(o)}</span></button>`).join('')}</div><div class="small muted" style="margin-top:14px">אפשר לשנות בחירה כל עוד ההצבעה פתוחה.</div></section>`);
