...
window.projectorStart=function(prefill=qs.get('code')||''){
  ...
  document.getElementById('projectBtn').onclick=()=>{const code=document.getElementById('projectorCode').value.trim();if(!code)return;location.href=`/projector?code=${encodeURIComponent(code)}`};
}

async function projectorRoom(code){
  let room=null;
  const poll=async()=>apiGet(code,{});
  const render=()=>{const q=questionAt(room.questionIndex);const joinUrl=`${base()}/join?code=${encodeURIComponent(room.code)}`;
    if(room.finished){
...
window.home=function(){if(window.__stopPolling)window.__stopPolling();location.href='/';}
